...
def main():
    ...
    assert tuple(assignment_df.index) == tuple(results_df.index)

    if options.by_true_labels:
        for label in all_labels:
            if label not in results_df.columns:
                results_df[label] = [0 for x in range(results_df.shape[0])]
        drop_labels = all_labels - set(results_df.columns)
        results_df = results_df.drop(sorted(drop_labels), axis=1)
    ...
    metrics_df.to_csv(join(out_dir, 'rank_cell_type_metrics.tsv'), sep='\t')
    with open(join(out_dir, 'label_to_pr_curve.json'), 'w') as f:
        json.dump(label_to_pr_curve, f, indent=4)
